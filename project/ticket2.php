...
<head>
	...
	$movieId=$_SESSION['movieId'];
		...
		if(isset($_GET['var'])){
			...
			$a=$_GET['var'];
			$sql="select * from hall2 where seatnumber='$a' ";
			...
			$time1="B1-1";
			$sql="update hall2 set name ='$name',email='$email',date='$date' where seatnumber='$a' ";
			$result=mysqli_query($conn,$sql);
		...
		<div class="wrapper">
			...
			for ($i=1;$i<=9;$i++){
				...
				<div>
					...
					<?php
					$time1="A".$i;
					$sql="select * from hall2 where seatnumber='$time1' ";
					$result= mysqli_query($conn, $sql);
					$res=mysqli_fetch_assoc($result);
					if($res['name']=="notbooked"){
						?>
